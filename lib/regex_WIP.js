/**
 * Trying to get support for RE2JS, which puts an upper bound on how long it can run for. I.e. if passed a very badly formatted email, it could run for over a minute; unless using re2. 
 * But not all tests pass, and I'm not sure why. The current hunch is the one modifier it's not yet supporting: /u
 */
...
class RegexList {
  constructor() {
    this.quoteHeadersRegex = this.buildRe2([
      ...
      /^\s*((W\sdniu|Dnia)\s[\s\S]+?(pisze|napisał(\(a\))?):)$/m, // W dniu DATE, NAME <EMAIL> pisze|napisał:
  ...
  buildRe2(regexList) {
    return regexList.map((regex) => {
        ...
        if( parts.length>1 ) {
            ...
            if( lastPart.includes('m') && lastPart.includes('i') ) {
                options = RE2JS.CASE_INSENSITIVE | RE2JS.MULTILINE;
            }
            else if( lastPart.includes('m') ) options = RE2JS.MULTILINE;
            parts.pop();
        }
        const regexStr = parts.join('/');

        
        const re = RE2JS.compile(regexStr, options);
        re.test = (str) => re.matcher(str).find()>0
        return re;
    });
  }
}
